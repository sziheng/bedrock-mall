...
<div class="form-group">
    ...
    <div class="col-sm-5">
        <input type="text" name="goodssn" id="goodssn" class="form-control hasoption" value="{{$good['goodssn']}}" @if($good['hasoption'])readonly @endif/>
    </div>

    <label class=" col-sm-1 control-label">条码</label>
    <div class="col-sm-5">
        <input type="text" name="productsn" id="productsn" class="form-control hasoption" value="{{$good['productsn']}}" />
    </div>
</div>
<div class="form-group">
    ...
    <div class="col-sm-11">
        <div class="input-group">
            <input type="text" name="weight" id="weight" class="form-control hasoption" value="{{$good['weight']}}" />
...
<div class="form-group">
    ...
    <div class="col-sm-11">
        ...
        <input type="text" name="total" id="total" class="form-control hasoption" value="{{$good['total']}}"  style="width:150px;display: inline;margin-right: 20px;" />
        <label class="checkbox-inline">
            <input type="checkbox" id="showtotal" value="1" name="showtotal" @if($good['showtotal'] == 1) checked @endif />显示库存
        </label>
        <span class="help-block">商品的剩余数量, 如启用多规格{if com('virtual')}或为虚拟卡密产品{/if}，则此处设置无效.</span>
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-11">
        <label for="totalcnf1" class="radio-inline"><input type="radio" name="totalcnf" value="0" id="totalcnf1" @if(!$good || $good['totalcnf'] == 0 ) checked="true" @endif /> 拍下减库存</label>
        <label for="totalcnf2" class="radio-inline"><input type="radio" name="totalcnf" value="1" id="totalcnf2"  @if($good && $good['totalcnf'] == 1) checked="true" @endif /> 付款减库存</label>
        <label for="totalcnf3" class="radio-inline"><input type="radio" name="totalcnf" value="2" id="totalcnf3"  @if($good && $good['totalcnf'] == 2) checked="true" @endif /> 永不减库存</label>
    </div>
</div>

<div class="form-group">
    <div class="col-sm-11" style='padding-left:95px;'>
        <label class="checkbox-inline">
            <input type="checkbox" id="hasoption" value="1" name="hasoption" @if($good['hasoption'] == 1) checked @endif />启用商品规格
        </label>
        <span class="help-block">启用商品规格后，商品的价格及库存以商品规格为准,库存设置为0则会到”已售罄“中，手机也不会显示, -1为不限制</span>
    </div>
</div>

<div id='tboption' style="padding-left:15px;@if($good['hasoption'] !=1) display:none @endif" >
...
<script>
    $(function(){
        ...
        $(".spec_item_thumb").find('i').click(function(){
            ...
            group.find('img').attr('src',"{php echo WESHOP_LOCAL}static/images/nopic100.jpg");
    ...
    function selectSpecItemImage(obj){
        util.image('',function(val){
            ...
            group.find(':hidden').val(val.attachment), group.find('i').show().unbind('click').click(function(){
                $(obj).attr('src',"{php echo WESHOP_LOCAL}static/images/nopic100.jpg");
                group.find(':hidden').val('');
                group.find('i').hide();
                $(obj).popover('destroy');
            });
        ...
        for(var m=0;m<len;m++){
            ...
            function refreshDiscount() {
                ...
                for (var i = 0; i < newlen; i++) {
                    ...
                    var val ={ id : "",title:titles,@foreach($levels as $level)@if($level['key'] == 'default') level{{$level['key']}}: '',@else level{{$level['id']}}: '',@endif @endforeach costprice : "",productprice : "",marketprice : "",weight:"",productsn:"",goodssn:"",virtual:virtuals };
            ...
            function refreshCommission() {
                var commission_level = '{{json_encode($commissionLevels)}}';
                var html = '<table class="table table-bordered table-condensed"><thead><tr class="active">';
                var specs = [];
                ...
                for (var i = 0; i < newlen; i++) {
                    ...
                    var val = {
                            id: "",
                            title: titles,
                                @foreach($commissionLevels as $level) @if($level['key'] == 'default') level{{$level['key']}}: '',
                                @else level{{$level['id']}}: '',
                                @endif @endforeach costprice:
                                    "",
                            productprice:
                                "",
                            marketprice:
                    ...
                    @foreach($commissionLevels as $level)
                        hh += '<td>';
                    var level_temp = @if($level['key'] =='default') level{{$level['key']}}@else level{{$level['id']}} @endif;
                    if (len >= i && typeof (level_temp) != 'undefined') {
                        if ('{{$level['key']}}' == 'default') {
                            ...
                            for (var li = 0; li < '{{$shopset_level}}'; li++) {
                                if (typeof (level_temp[li]) != "undefined") {
                                    hh += '<input data-name="commission_level_{{array_get($level,'id','0')}}_' + ids + '"  type="text" class="form-control commission_level{{array_get($level,'id','0')}} commission_level{{array_get($level,'id','0')}}_' + ids + '" value="' + $(level_temp[li]).val() + '" style="display:inline;width: ' + 96 / parseInt({{$shopset_level}}) + '%;"/> ';
                                }
                                else {
                                    hh += '<input data-name="commission_level_{{array_get($level,'id','0')}}_' + ids + '"  type="text" class="form-control commission_level{{array_get($level,'id','0')}} commission_level{{array_get($level,'id','0')}}_' + ids + '" value="" style="display:inline;width: ' + 96 / parseInt({{$shopset_level}}) + '%;"/> ';
                                }
                            }
                        }
                    }
